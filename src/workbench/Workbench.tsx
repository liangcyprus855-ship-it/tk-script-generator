...
export default function Workbench(_props: {initialSettings?: any}) {
  ...
  {accountOpen&&<AccountModal account={account} records={records} onLogin={authenticate} onRegister={authenticate} onSendCode={async email=>{await request('/api/account/send-code','',{email},{retryOnNetwork:3});}} onLogout={()=>{const auth=token;clearSession();setAccountOpen(false);void request('/api/account/logout',auth,{}).catch(()=>{});}} onOpenRecharge={()=>{setAccountOpen(false);setRechargeOpen(true);}} onOpenHistory={()=>{setAccountOpen(false);setHistoryOpen(true);}} onClose={()=>setAccountOpen(false)}/>}
