...
function isAdmin(request, env) { const supplied = text(request.headers.get("x-admin-token") || authToken(request)); return Boolean(text(env.ADMIN_TOKEN) && supplied && supplied === text(env.ADMIN_TOKEN)); }
...
const adminPage = `<!doctype html><meta charset="utf-8"><title>TK脚本生成器后台</title><style>body{font-family:system-ui;margin:32px;background:#f6f7fb;color:#172033}button,input{padding:8px 12px;margin:4px}table{border-collapse:collapse;background:white;width:100%}td,th{padding:10px;border:1px solid #ddd;text-align:left}.ok{color:#087f5b}</style><h1>TK脚本生成器 · 充值审核</h1><p>管理员令牌：<input id="token" type="password" size="40"><button onclick="load()">加载待审核订单</button></p><div id="msg"></div><table><thead><tr><th>订单</th><th>账户</th><th>金额</th><th>积分</th><th>备注</th><th>操作</th></tr></thead><tbody id="rows"></tbody></table><script>async function load(){const t=document.querySelector('#token').value;const r=await fetch('/api/admin/orders',{headers:{'x-admin-token':t}});const d=await r.json();if(!r.ok){msg.textContent=d.error||'加载失败';return}rows.innerHTML=d.orders.map(o=>'<tr><td>'+o.id+'</td><td>'+o.email+'</td><td>'+(o.amount_fen/100).toFixed(2)+' 元</td><td>'+o.credits+'</td><td>'+((o.note||'')+'').replaceAll('<','&lt;')+'</td><td><button onclick="act(\''+o.id+'\',\'approve\')">发放积分</button><button onclick="act(\''+o.id+'\',\'reject\')">驳回</button></td></tr>').join('')}async function act(id,a){const t=document.querySelector('#token').value;const r=await fetch('/api/admin/orders/'+id+'/'+a,{method:'POST',headers:{'x-admin-token':t}});const d=await r.json();msg.textContent=d.error||d.message||'完成';load()}</script>`;
...
export default { async fetch(request, env) {
  ...
  try {
    ...
    if (route === "/api/admin/orders" && request.method === "GET") { if (!isAdmin(request, env)) return json({ error: "管理员令牌无效" }, 403); await ensureProofTable(env); const rows = await env.DB.prepare("SELECT o.*, a.email, p.note FROM orders o JOIN accounts a ON a.id=o.account_id LEFT JOIN payment_proofs p ON p.order_id=o.id WHERE o.status='proof_submitted' ORDER BY o.created_at ASC").all(); return json({ orders: rows.results || [] }); }
    const adminAction = route.match(/^\/api\/admin\/orders\/([^/]+)\/(approve|reject)$/); if (adminAction && request.method === "POST") { if (!isAdmin(request, env)) return json({ error: "管理员令牌无效" }, 403); const order = await env.DB.prepare("SELECT * FROM orders WHERE id=?").bind(adminAction[1]).first(); if (!order || order.status !== "proof_submitted") return json({ error: "订单不存在或已处理" }, 404); if (adminAction[2] === "reject") { await env.DB.prepare("UPDATE orders SET status='rejected' WHERE id=?").bind(order.id).run(); return json({ message: "订单已驳回" }); } const account = await env.DB.prepare("SELECT credits FROM accounts WHERE id=?").bind(order.account_id).first(); const balance = Number(account?.credits || 0) + Number(order.credits); await env.DB.batch([env.DB.prepare("UPDATE orders SET status='paid',paid_at=? WHERE id=? AND status='proof_submitted'").bind(now(), order.id), env.DB.prepare("UPDATE accounts SET credits=? WHERE id=?").bind(balance, order.account_id), env.DB.prepare("INSERT INTO ledger(id,account_id,type,amount,balance,description,created_at) VALUES(?,?,?,?,?,?,?)").bind(crypto.randomUUID(), order.account_id, "recharge", order.credits, balance, `人工审核到账：${order.package_id}`, now())]); return json({ message: `已发放 ${order.credits} 积分`, balance }); }
