...
export default { async fetch(request, env) {
  ...
  try {
    ...
    const proof = route.match(/^\/api\/billing\/orders\/([^/]+)\/submit-proof$/); if (proof && request.method === "POST") { const user = await requireUser(request, env); if (!user) return json({ error: "请先登录" }, 401); await request.json().catch(() => ({})); const order = await env.DB.prepare("SELECT * FROM orders WHERE id=? AND account_id=?").bind(proof[1], user.id).first(); if (!order || order.provider !== "alipay_personal") return json({ error: "充值订单不存在" }, 404); await env.DB.prepare("UPDATE orders SET status='proof_submitted' WHERE id=? AND status='pending'").bind(order.id).run(); return json({ ok: true, message: "付款备注已提交，管理员确认后会增加积分" }); }
