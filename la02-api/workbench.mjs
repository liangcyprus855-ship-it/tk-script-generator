...
export function mountWorkbench(app,pool,userFrom,root=path.dirname(fileURLToPath(import.meta.url))) {
  ...
  app.get('/api/billing/channels',route(async(_req,res)=>res.json({channels:channels(),minAmountFen:500})));
  ...
  app.post('/api/billing/orders',route(async(req,res,user,next)=>{
    ...
    if(!Number.isSafeInteger(amount)||amount<500||amount>1000000)return res.status(400).json({error:'充值金额须为 ¥5 至 ¥10000'});
