...
The API requires its existing PostgreSQL account/order tables, PG connection variables and `MIMO_API_KEY`. Startup creates `generation_jobs` if absent. API port defaults to 38127 on loopback.

`POST /api/generate` accepts product, region, targetAudience, features, duration, optional image and a unique requestId. The server computes the price. An account lock and a unique account/requestId pair prevent duplicate charges; only one running job per account and four globally are allowed. Clients poll the returned jobId; `/api/generation-jobs/latest` resumes the last task on relaunch.
